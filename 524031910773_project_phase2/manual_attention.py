...
def qwen3_5_attention_forward(
    ...
    if past_key_values is not None:
        # 过去长度，计算位置偏移
        past_length = past_key_values.get_seq_length(layer_idx)
        current_length = hidden_states.shape[1]
        # 生成 [past, past + 1, ..., past + current - 1]
        cache_position = torch.arange(past_length, past_length + current_length, device=hidden_states.device)

        cache_kwargs = {"cache_position": cache_position}
        key_states, value_states = past_key_values.update(key_states, value_states, layer_idx, cache_kwargs = cache_kwargs)
