    ...
    def _rebuild_index(self) -> None:
        ...
        for fn in os.listdir(self.root_dir):
            ...
            try:
                ...
                warnings.warn(f"Failed to load SSD cache file {path}: {e}")
                continue
...
class TieredPrefixCache:
    ...
    def lookup(
        ...
        best_mem_key = None
        best_mem_len = 0
        target_tuple = tuple(token_ids)
        target_len = len(token_ids)
        
        # 参考phase3进行排除不匹配的key
        for cached_tokens in self._mem.keys():
            cached_len = len(cached_tokens)
            if cached_len > target_len:
                continue
                
            if cached_len > best_mem_len and cached_tokens == target_tuple[:cached_len]:
                best_mem_len = cached_len
                best_mem_key = cached_tokens
                
        best_ssd_key = None
        best_ssd_len = 0
        for cached_tokens in self._disk.keys():
            cached_len = len(cached_tokens)
            if cached_len > target_len:
                continue
                
            if cached_len > best_ssd_len and cached_tokens == target_tuple[:cached_len]:
                best_ssd_len = cached_len
                best_ssd_key = cached_tokens
                
        # 没匹配到
        if best_mem_len == 0 and best_ssd_len == 0:
            self.misses += 1
            return 0, None
        ...
        if matched_len == target_len:
            matched_len = target_len - 1
